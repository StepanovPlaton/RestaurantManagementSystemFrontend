...
function stepIndex(step: ClientUiStep): number {
  return CLIENT_UI_STEPS.findIndex((s) => s.key === step);
}
...
export function OrderTrackingStepper({
  ...
  const currentStep = toClientStep(currentStatusId);
  ...
  return (
    ...
    >
      {CLIENT_UI_STEPS.map((step, index) => {
